...
export function oauthIssuer(req: NextRequest): string {
  return normalizeNoTrailingSlash(
    process.env.OAUTH_ISSUER || requestOrigin(req)
  );
}
...
export function normalizeRequestedOauthScope(raw: string): {
  ok: boolean;
  scope?: string;
  reason?: string;
} {
  ...
  if (unsupported) {
    return { ok: false, reason: `unsupported scope: ${unsupported}` };
  }
...
function decodeSignedPayload(token: string): {
  ...
  try {
    ...
    if (header.alg !== oauthSigningAlgorithm()) {
    ...
    if (!valid) {
      return { ok: false, reason: "bad_signature_or_unknown_kid" };
    }
...
function verifyAuthorizationCode(code: string): {
  ok: boolean;
  reason?: string;
...
export function verifyOAuthAccessToken(
  ...
  if (!audience.includes(oauthResourceUrl(req))) {
...
export function validateAuthorizeParams(
  ...
  const requestedScope = normalizeRequestedOauthScope(
    params.get("scope") || ""
  );
  ...
  if (!requestedScope.ok) {
    return requestedScope.reason || "invalid scope";
  }
  return "";
}
...
export async function handleAuthorize(req: NextRequest): Promise<Response> {
  if (!ownerCode()) {
    return html(
      "<h1>OAuth setup required</h1><p>Set OAUTH_OWNER_CODE before using OAuth.</p>",
      500
    );
  }
  if (req.method === "GET") {
  ...
  const normalizedScope = normalizeRequestedOauthScope(
    params.get("scope") || ""
  );
  if (!normalizedScope.ok || !normalizedScope.scope) {
    return html(
      renderAuthorizePage(
        params,
        req,
        normalizedScope.reason || "Invalid scope"
      ),
      400
    );
  }
...
async function handleAuthorizationCodeGrant(
  ...
  if (!verifiedCode.ok || !verifiedCode.payload) {
    return oauthJson(
      {
        error: "invalid_grant",
        error_description: verifiedCode.reason || "Invalid code",
      },
      400
    );
  }
  ...
  const requestRedirectUri =
    params.get("redirect_uri") || entry.redirect_uri;
  if (
    requestClientId !== entry.client_id ||
    requestRedirectUri !== entry.redirect_uri
  ) {
    return oauthJson(
      {
        error: "invalid_grant",
...
async function handleRefreshGrant(
  ...
  if (rawScope !== null) {
    const normalized = normalizeRequestedOauthScope(rawScope);
    if (!normalized.ok || !normalized.scope) {
      return oauthJson(
        {
          error: "invalid_scope",
          error_description: normalized.reason || "Invalid scope",
        },
        400
      );
    }
  ...
  if (!rotated.ok) {
    return oauthJson(
      {
        error: rotated.reason === "invalid_scope" ? "invalid_scope" : "invalid_grant",
        error_description:
          rotated.description || `Refresh grant ${rotated.reason}`,
      },
      400
    );
  }
  ...
  const access = issueAccessToken({
    ...
    tokenSubject: rotated.record.subject,
  });
  return tokenResponse({
    accessToken: access.accessToken,
    expiresIn: access.expiresIn,
    scope: rotated.record.scope,
    refreshCredential: rotated.credential,
  });
}
...
export async function handleToken(req: NextRequest): Promise<Response> {
  ...
  if (grantType === "authorization_code") {
    return handleAuthorizationCodeGrant(params, req);
  }
  if (grantType === "refresh_token") {
    return handleRefreshGrant(params, req);
  }
...
export async function handleRevoke(req: NextRequest): Promise<Response> {
  ...
  if (credential) await revokeRefreshCredentialFamily(credential);
