...
export const VERIFY_OPERATING_GUIDE = {
  ...
  startupProtocol: [
    ...
    "Use purr_work_session_start for npm run dev or equivalent, then use snapshot, act, screenshot, artifacts, inspect, and diagnostics against the same persistent Pursr browser state. PNG remains the default screenshot format; JPEG, WebP, GIF, AVIF, TIFF, video, audio, and unknown binary artifacts remain available. Inline screenshots and metadata-only artifact responses are the default so normal browser work does not trigger ChatGPT file-materialization approval. Resource-link attachments remain an explicit includeAttachments=true capability.",
