...
export const VERIFY_OPERATING_GUIDE = {
  ...
  startupProtocol: [
    ...
    "Use purr_upload_file for a ChatGPT connector file, absolute destination path, and expected SHA-256. mode=auto returns connector downloads immediately as durable jobs; poll purr_get_job_status until terminal. The tool accepts opaque bytes of any format, streams without an application-level size cap, creates parent directories, deduplicates identical destination+SHA retries, and atomically replaces an existing destination only after checksum verification.",
  ...
  hardRules: [
    ...
    "Binary upload treats file content as opaque bytes: no extension whitelist, MIME whitelist, or application-level byte cap is applied. Connector downloads default to durable async jobs, identical destination+SHA retries reuse one active job, conflicting hashes are rejected while the destination is owned, and SHA-256 mismatch leaves the previous destination unchanged.",
