...
5. For binary transfer, call `purr_upload_file` with the connector file, an absolute destination, and the expected SHA-256. Auto mode always returns a durable async job for connector downloads and mounted local files; poll `purr_get_job_status` until terminal.
...
- Binary upload accepts opaque bytes of every format and applies no extension, MIME, or application-level byte cap. Auto mode must use durable async jobs for every source type, identical normalized destination+SHA retries must reuse one active job across process boundaries, conflicting active hashes must fail without starting another stream, interrupted destination ownership must be reclaimed safely, signed download URLs must never be persisted, and checksum mismatch or cancellation must leave the destination unchanged.
- Self-hosted systemd restart must use an external delayed handoff so the current job persists its terminal state and releases the canonical project lock before the Verify MCP process is replaced. Verify health after reconnection.
