...
- RS256 access tokens with stable `kid`, public JWKS, issuer, subject, audience, client, scopes, `iat`, `nbf`, `exp`, and `jti` claims.
...
- Bearer credentials never accepted from query strings.
...
- Wrong issuer, audience, resource, redirect URI, client, PKCE verifier, expired token, and authorization-code replay fail closed.
- Read-only credentials cannot create/cancel jobs or create/revoke share links.
- OAuth credentials cannot authenticate non-canonical REST or MCP aliases.
...
3. Validate discovery, authorization, token exchange, scoped tool calls, expiry, and reconnect behavior.
4. Keep the previous release available for rollback; rolling back invalidates newly issued RS256 credentials and requires reconnecting.
